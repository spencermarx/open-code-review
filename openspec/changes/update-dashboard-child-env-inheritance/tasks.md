...
- [ ] 1.1 Capture the child-env snapshot in
...
- [ ] 1.2 Make `childEnvBase` a required `StartServerOptions` field; the dev
...
- [ ] 2.1 Rewrite `packages/dashboard/src/server/socket/env.ts`: delete
      ...
      `injected`
- [ ] 2.2 Implement the denylist constants with per-entry rationale
...
- [ ] 2.3 Add `describeChildEnvPosture()` generating human-readable posture
      text from the same constants
- [ ] 2.4 Unit tests: provider vars pass through; `OCR_*` stripped, fresh
...
- [ ] 3.1 Converge all spawn paths on `buildChildEnv` with the holder's
      ...
      `post-handler.ts`
- [ ] 3.2 Fix the forward-resume sweep spawn in `server/index.ts` to pass
      the builder's env (today it inherits the full mutated dashboard env)
- [ ] 3.3 Add ESLint `no-restricted-properties` ban on `process.env` scoped
      to the spawn modules; fix any fallout
- [ ] 3.4 Adapter/unit tests: spawn env equals builder output; no spawn site
      reads `process.env`

## 4. Shared config layer

- [ ] 4.1 Add `packages/shared/config/src/dashboard-config.ts` with
...
- [ ] 4.2 Replace the regex `ai_cli` read in
      `services/ai-cli/index.ts` with the shared layer; wire the startup
      warn-once notice for the retired `env_passthrough` key
- [ ] 4.3 Unit tests: parse/default/malformed cases; warn-once notice; key
      absent from `DashboardConfig` type

## 5. Observability

- [ ] 5.1 Startup line: snapshot ISO-8601 timestamp, source, removed names,
      restart-remedy sentence
- [ ] 5.2 Per-execution log header from the same `buildChildEnv` result:
      `env: shell snapshot <ts> (<source>) minus [...] plus [...]`
- [ ] 5.3 Failure-time hint on nonzero child exit in the run-detail error
      surface: removed names, snapshot timestamp, restart remedy; explicit
      `NODE_OPTIONS` causality sentence when applicable
- [ ] 5.4 `ocr doctor` posture section from `describeChildEnvPosture()`
      including the `env -u VAR ocr dashboard` hint and naming
      `NODE_OPTIONS` as the one intentional shell-parity break
- [ ] 5.5 Tests: names-only invariant (no values in any surface); header
      derived from the actual spawn's builder result

## 6. Docs and template

- [ ] 6.1 README: replace the dashboard env section with the two-sentence
      contract, the restart semantics, and the `env -u` hint
- [ ] 6.2 `packages/agents/skills/ocr/assets/config.yaml` template: document
      `dashboard.ai_cli`; no env keys; run `nx run cli:update` to sync
- [ ] 6.3 Changelog notes: the two parity-restoring narrowings (child
      `NODE_ENV`, forward-resume env); credit PR #56 (@thunderock) for the
      salvaged config-layer approach and env hardening

## 7. Verification

- [ ] 7.1 `nx run-many -t typecheck` and `-t lint` green across all projects
- [ ] 7.2 Full test suites for `dashboard`, `config`, `cli` green
- [ ] 7.3 End-to-end smoke: export a non-allowlisted variable (e.g.
      ...
      reports the delta
- [ ] 7.4 `openspec validate update-dashboard-child-env-inheritance --strict`
